...
class CAModel(tf.keras.Model):
    ...
    def __init__(self, NO_NEIGHBORS, NO_CHANNELS, NO_CLASSES, H, W, add_noise=True, model_complexity='simplest', dead_pixels=False):
        super().__init__()
        ...
        if model_complexity == 'simplest':
            self.update_state = tf.keras.Sequential([
                Conv2D(NO_CHANNELS, 2 * NO_NEIGHBORS + 1, activation=None, padding="SAME"),
            ])
        elif model_complexity == 'middle':
            self.update_state = tf.keras.Sequential([
                Conv2D(40, 2 * NO_NEIGHBORS + 1, activation=tf.nn.relu, padding="SAME"),
                Conv2D(NO_CHANNELS, 1, activation=None, padding="SAME"),
            ])
        else:
            self.update_state = tf.keras.Sequential([
                Conv2D(80, 2 * NO_NEIGHBORS + 1, activation=tf.nn.relu, padding="SAME"),
...
def batch_l2_loss(ca, x, y, label_vector, NO_CLASSES):
    ...
    x is the current CA state vector. its shape is (batch_size, height, width, no_channels).
    y is the correct label out of NO_CLASSES possibilities. its shape is (batch_size, height, width, NO_CLASSES) (one-hot)
    label_vector is the correct class out of NO_CLASSES possibilities. its shape is (batch_size, )
    returns the mean of the loss function and the mean for each class
    '''
...
def get_model(folder, id_run, i, NO_NEIGHBORS, NO_CHANNELS, NO_CLASSES, H, W, ADD_NOISE, model_complexity, dead_pixels=False):
    '''
    Gets the models parameters
    '''
    ca = CAModel(NO_NEIGHBORS, NO_CHANNELS, NO_CLASSES, H, W, add_noise=ADD_NOISE, model_complexity=model_complexity, dead_pixels=dead_pixels)
...
def train_step(trainer, ca, x, y, y_label, TR_EVOLVE, NO_CLASSES, MutateTrainingQ=False):
    ...
    y is the correct label out of NO_CLASSES possibilities. its shape is (batch_size, height, width, NO_CLASSES) (one-hot)
    TR_EVOLVE is an integer specifying how many iterations we let the system evolve. (note that we subdivide this number)
    '''
